...
function doPost(e) {
  try {
    ...
    if (sheet.getLastRow() === 0) {
      // Row 1: Title Banner
      sheet.getRange('A1:F1').merge().setValue('JHONY & SANDRA — WEDDING RSVP');
      ...
      sheet.getRange('A2:B2').merge().setValue('TOTAL ATTENDING PEOPLE:');
      ...
      sheet.getRange('C2:D2').merge().setFormula('=SUMIF(C5:C, "Yes*", D5:D)').setFontWeight('bold').setFontSize(14).setBackground('#242424').setFontColor('#c9a84c').setHorizontalAlignment('center');
      sheet.getRange('E2').setValue('Total Responses:');
      ...
      sheet.getRange('F2').setFormula('=COUNTA(A5:A)').setFontWeight('bold').setFontSize(11).setBackground('#242424').setFontColor('#ffffff').setHorizontalAlignment('center');

      // Row 3: Empty space
      sheet.getRange('A3:F3').setBackground('#f5f5f5');
